<template>
  <div
    class="brick"
    v-for="data in discusses.data.pageDiscussList"
    :key="data.did"
  >
    <el-card class="brick__inner" @click="$router.push('/article/' + data.did)">
      <div class="brick__info-userInfo flex items-center ml-2">
        <el-avatar :size="30" :src="data.avatar"></el-avatar>
        <span>
          <div class="ml-2">{{ data.nickname }}</div>
          <div class="ml-2">{{ data.timeStr }}</div>
        </span>
      </div>
      ...
      <div class="brick__info">
        ...
        <div class="brick__info-footer">
          <div class="brick__info-watch flex justify-around items-center">
            <SvgIcon name="love" size="12" class="mr-1"></SvgIcon>
            <span class="mr-1">{{ data.likeNum }}</span>
            <SvgIcon name="laugh" size="12" class="mr-1"></SvgIcon>
            <span class="mr-1">{{ data.collectNum }}</span>
            <SvgIcon name="icon_message" size="12" class="mr-1"></SvgIcon>
            <span class="mr-1">{{ data.commentNum }}</span>
          ...
          <span class="float-right brick__info-watch">
            ...
            <span class="mr-1">{{ data.watchNum }}</span>
          </span>
        </div>
      </div>
    </el-card>
...
<script setup>
import { ref } from 'vue';
import { discussList } from '@/api/discuss';
import { useArticleStore } from '@/store/article';

const discusses = useArticleStore();
discusses.discussListData({ page: 1 });
setTimeout(() => {
  console.log(discusses.data.pageDiscussList);
}, 3000);
// const discusses = ref([]);
// discussList({ page: 1 }).then(res => {
//   discusses.value = res.data.pageDiscussList;
// });

// let communitys = ref([
//   {
//     avatar: '/img/user-placeholder.webp',
//     picture: '/img/feature-img1.png',
//     id: 666,
//     title: '第一篇文章',
//     author: 'author',
//     sendTime: '今天',
