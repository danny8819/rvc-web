  ...
  <div class="team">
    ...
    <div id="banner">
      <div class="img-list img-wrapper">
        <div class="img-box" v-for="(member,i) in members" :id="i!=members.length-1?null:'last-img-box'">
          <div class="info">
    ...
    <div class="btn-group">
      <button class="last btn" @click="throttle(()=>clickFun('last'),animationTime*1000)">
      ...
      <button class="next btn" @click="throttle(()=>clickFun('next'),animationTime*1000)">
...
export default {
  ...
  async mounted() {
    ...
    this.lastImgBox.style.transform = `translateX(`+-1*this.leftInit+`vw)`
    ...
    clickFun(flag){
      ...
      if (flag == 'next') {
          ...
          if (Math.abs(this.index) === this.imgBoxCount) {
            ...
            this.imgBoxList.forEach(item => {
              if (item.id === 'last-img-box') {
                item.style.transform = `translateX(`+-1*this.leftInit+`vw)`
              } else {
            ...
            if (this.imgBoxList[0].id == 'last-img-box') {
              ...
              */
              ...
              this.imgBoxList[0].style.transform = `translateX(`+this.leftInit+`vw)`
            }
          }
        ...
        if (this.imgBoxList[0].id === 'last-img-box' && this.index !== 0) {
          ...
          this.imgBoxList[0].style.transform = `translateX(`+-2*this.leftInit+`vw)`
        } else if (this.index < 0) {
          // 这种情况与点击next按钮出现的回退现象一致
          this.imgBoxList[0].style.transform = 'none'
        } else {
          // 正常情况下，点击last，则将最右侧的图片移到最前
          this.imgBoxList[0].style.transform = `translateX(`+-1*this.leftInit+`vw)`
        }
        this.imgListOne.style.left = this.imgBoxLength * this.index + "vw";
